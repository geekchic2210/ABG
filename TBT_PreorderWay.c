...
void preorderTraversal(struct Node* root) {
    ...
    while (current != NULL) {
        ...
        // If the current node has no left child, but it's threaded, move to the threaded right child
        else if (current->isThreaded == 1) {
            current = current->right;
        }
        // If the current node has neither left child nor threaded right child, move to the actual right child
        else {
            current = current->right;
            // Move up until finding a node with a threaded right child or a right child
